...
function brandVars(brand) {
    ...
    const vars = [];
    if (p)
        vars.push(`--brand-primary:${p};`);
    if (s)
        vars.push(`--brand-secondary:${s};`);
    if (a)
        vars.push(`--brand-accent:${a};`);
    return vars.join(' ');
}
/** Logo veya işletme adı header bloğu */
function logoOrName(menu, cls = '') {
    const logoUrl = menu.logoUrl;
    if (logoUrl) {
 ...
 */
function wrapHtml(title, bodyClass, cssVars, extraCss, body, brand) {
...
function productImg(imageUrl, alt, cls = '') {
    if (!imageUrl)
        return `<div class="no-img ${cls}" aria-hidden="true">🍽</div>`;
    return `<img src="${escHtml(imageUrl)}" alt="${escHtml(alt)}" class="${cls}" loading="lazy"/>`;
}
// ─── THEME 01: Classic List ─────────────────────────────────────────────────
function theme01({ menu, categories, productsByCategory }) {
    const rows = categories
        ...
        .join('');
    return wrapHtml(menu.businessName || 'Our Menu', '', '--accent:#c0392b;', `body{max-width:700px;margin:0 auto;padding:1rem;background:#fefefe;color:#222;}
    ...
    .no-img{display:none;}`, `<header><h1>${escHtml(menu.businessName || 'Our Menu')}</h1></header>
    <main>${rows || '<p style="text-align:center;padding:2rem;color:#888;">No items yet.</p>'}</main>`);
}
// ─── THEME 02: Card Grid ────────────────────────────────────────────────────
function theme02({ menu, categories, productsByCategory }) {
    const sections = categories
        ...
        .join('');
    return wrapHtml(menu.businessName || 'Our Menu', '', '--accent:#2980b9;--radius:12px;', `body{background:#f4f7fb;color:#222;padding:0;}
    ...
    .price{font-weight:700;color:var(--accent);}`, `<header><h1>${escHtml(menu.businessName || 'Our Menu')}</h1></header>
    <main>${sections || '<p style="text-align:center;padding:2rem;color:#888;">No items yet.</p>'}</main>`);
}
// ─── THEME 03: Minimal Typography ───────────────────────────────────────────
function theme03({ menu, categories, productsByCategory }) {
    const sections = categories
        .map((cat) => {
        const prods = productsByCategory[cat.id] || [];
        if (!prods.length)
        ...
        .join('');
    return wrapHtml(menu.businessName || 'Menu', '', '--font-serif:Georgia,serif;', `body{background:#fff;color:#1a1a1a;max-width:640px;margin:0 auto;padding:2rem 1.5rem;}
    ...
    .item-price{font-family:var(--font-serif);font-size:1rem;}`, `<h1>${escHtml(menu.businessName || 'Menu')}</h1>
    <p class="tagline">Est. 2024</p>
    <main>${sections || '<p style="text-align:center;color:#ccc;">No items yet.</p>'}</main>`);
}
// ─── THEME 04: Dark Mode ─────────────────────────────────────────────────────
function theme04({ menu, categories, productsByCategory }) {
    const sections = categories
        .map((cat) => {
        const prods = productsByCategory[cat.id] || [];
        if (!prods.length)
        ...
        .join('');
    return wrapHtml(menu.businessName || 'Menu', '', '', `body{background:#0d0d0d;color:#e0e0e0;font-family:'Segoe UI',sans-serif;padding:0;}
    ...
    .dark-price{font-weight:700;color:#f39c12;white-space:nowrap;}`, `<header><h1>${escHtml(menu.businessName || 'Menu')}</h1></header>
    <main>${sections || '<p style="text-align:center;padding:2rem;color:#555;">No items yet.</p>'}</main>`);
}
// ─── THEME 05: Hero Sections ─────────────────────────────────────────────────
function theme05({ menu, categories, productsByCategory }) {
    const gradients = [
    ...
    const sections = categories
        ...
        .join('');
    return wrapHtml(menu.businessName || 'Menu', '', '', `body{background:#f8f8f8;color:#222;margin:0;}
    ...
    .no-items{color:#aaa;padding:.5rem;}`, `<div class="top-header"><h1>${escHtml(menu.businessName || 'Menu')}</h1></div>
    <main>${sections || '<p style="text-align:center;padding:2rem;color:#888;">No items yet.</p>'}</main>`);
}
// ─── THEME 06: Sidebar Navigation ────────────────────────────────────────────
function theme06({ menu, categories, productsByCategory }) {
    const navLinks = categories
        .map((cat) => `<a href="#cat-${escHtml(cat.id)}" class="nav-link">${escHtml(cat.name)}</a>`)
        .join('');
    const sections = categories
        .map((cat) => {
        const prods = productsByCategory[cat.id] || [];
        ...
        .join('');
    return wrapHtml(menu.businessName || 'Menu', '', '--sidebar:220px;--accent:#6c5ce7;', `body{display:flex;min-height:100vh;color:#222;background:#f9f9f9;}
    ...
    .empty{color:#ccc;}
    @media(max-width:600px){.sidebar{display:none;}.content{margin-left:0;}}`, `<nav class="sidebar"><h2>${escHtml(menu.businessName || 'Menu')}</h2>${navLinks}</nav>
    <div class="content">
...
function theme07({ menu, categories, productsByCategory }) {
    const pills = categories
        .map((cat) => `<a href="#cat-${escHtml(cat.id)}" class="pill">${escHtml(cat.name)}</a>`)
        .join('');
    const sections = categories
        .map((cat) => {
        const prods = productsByCategory[cat.id] || [];
        ...
        .join('');
    return wrapHtml(menu.businessName || 'Menu', '', '--accent:#e17055;', `body{background:#fff8f5;color:#222;margin:0;}
    ...
    .empty{color:#ccc;}`, `<div class="top"><h1>${escHtml(menu.businessName || 'Menu')}</h1></div>
...
function theme08({ menu, categories, productsByCategory }) {
    const sections = categories
        .map((cat) => {
        const prods = productsByCategory[cat.id] || [];
        const cards = prods
        ...
        .join('');
    return wrapHtml(menu.businessName || 'Menu', '', '--accent:#00b894;', `body{background:#fafafa;color:#222;margin:0;}
    ...
    .empty{color:#ccc;}`, `<header><h1>${escHtml(menu.businessName || 'Menu')}</h1></header>
    <main>${sections || '<p style="text-align:center;padding:2rem;color:#ccc;">No items yet.</p>'}</main>`);
}
// ─── THEME 09: Compact Menu ───────────────────────────────────────────────────
function theme09({ menu, categories, productsByCategory }) {
    const sections = categories
        .map((cat) => {
        const prods = productsByCategory[cat.id] || [];
        const rows = prods
        ...
        .join('');
    return wrapHtml(menu.businessName || 'Menu', '', '--accent:#2d3436;', `body{background:#fff;color:#222;max-width:600px;margin:0 auto;padding:1rem;font-size:.9rem;}
    ...
    .empty{color:#ccc;font-size:.8rem;padding:.25rem 0;}`, `<h1>${escHtml(menu.businessName || 'Menu')}</h1>
    <main>${sections || '<p style="text-align:center;padding:2rem;color:#ccc;">No items yet.</p>'}</main>`);
}
// ─── THEME 10: Premium ───────────────────────────────────────────────────────
function theme10({ menu, categories, productsByCategory }) {
    const navItems = categories
        .map((cat) => `<button class="tab-btn" onclick="showTab('cat-${escHtml(cat.id)}')">${escHtml(cat.name)}</button>`)
        .join('');
    ...
    return wrapHtml(menu.businessName || 'Menu', '', '--gold:#d4af37;--dark:#1a1a2e;--mid:#16213e;', `@keyframes fadeIn{from{opacity:0;transform:translateY(8px);}to{opacity:1;transform:translateY(0);}}
    ...
    .empty{color:#444;text-align:center;padding:2rem;}`, `<div class="prm-header">
...
function themeYeni01({ menu, categories, productsByCategory, brand }) {
    const emojiMap = {
    ...
    const tabsHtml = categories.map(c => `<button onclick="showCat('${c.id}')" class="tab-btn flex flex-col items-center gap-1 px-4 py-2 rounded-2xl bg-white/10 hover:bg-white/20 transition min-w-[70px]" id="btn-${c.id}">
    ...
    const cardsHtml = categories.map(c => {
        ...
        const cards = prods.map(p => `<div class="bg-white rounded-2xl overflow-hidden shadow-md flex flex-col">
...
function themeYeni02({ menu, categories, productsByCategory, brand }) {
    const pillsHtml = categories.map((c, i) => `<button onclick="scrollTo2('${c.id}')" class="pill-btn whitespace-nowrap px-5 py-2 rounded-full text-sm font-semibold border-2 transition ${i === 0 ? 'text-white' : 'bg-white border-gray-200 text-gray-700 hover:border-current'}" style="${i === 0 ? 'background:var(--primary);border-color:var(--primary)' : ''}">${c.name}</button>`).join('');
    const listHtml = categories.map(c => {
        const prods = productsByCategory[c.id] ?? [];
        const items = prods.map(p => `<div class="flex items-center gap-4 p-4 bg-white rounded-2xl shadow-sm">
         ${p.imageUrl
...
function themeYeni03({ menu, categories, productsByCategory, brand }) {
    const sectionsHtml = categories.map(c => {
        const prods = productsByCategory[c.id] ?? [];
        const cards = prods.map(p => `<div class="bg-white rounded-2xl overflow-hidden shadow-sm mb-4">
...
function renderTheme(templateKey, data) {
